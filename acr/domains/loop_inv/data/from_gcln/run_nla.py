...
if __name__ == '__main__':
    ...
    for p in pnames:
        ...
        # for i in [1]:
            ...
            for test in range(ntests):
                start_time = time.time()
                # if os.path.exists('tmp_res.pkl'):
                    # with open('tmp_res.pkl', 'rb') as f:
                        # res = dill.load(f)
                # else:
                res = problem[i].run(random_seed=random_seed)
                print('res:', res, len(res))
                print('res-types:', [type(r) for r in res])
                # with open('tmp_res.pkl', 'wb') as f:
                    # dill.dump(res, f)
                elapsed = time.time() - start_time
                total_time += elapsed
                total_tests += 1
                print('composed invariant-in', res, p)
                invariant, z3_vars = compose_invariant(res[0], res[1], res[2], res[3], p)
                print('composed invariant-out', invariant, z3_vars)
                print('composed invariant-types', type(invariant), type(z3_vars))
                print('invariant-types:', [type(i) for i in invariant])
                check_res, full_invariant, doc_invariant = check_nla_invariant(invariant, p, loop_idx, ctx_simplify=True)
                print('check_res:', check_res, type(check_res))
                print('full_invariant:', full_invariant, type(full_invariant))
                print('doc_invariant:', doc_invariant, type(doc_invariant))
