...
def multi_lin_eq_solve(data, and_span=2, or_span=2, or_reg=(0.001, 1.001, 0.1), and_reg=(1.0, 0.999, 0.1),
    ...
    if num_terms > 1:
        ...
        if True:  # valid_equality_found:
            ...
            model.weight.data = model.weight / torch.max(torch.abs(model.weight), dim=1, keepdim=True)[0]
            # print(model.weight)
            # for wi, weight in enumerate(model.weight):
                # weight /= torch.max(torch.abs(weight))
    ...
    # final_guess = filter_coeffs(np.asarray(coeffs, dtype=np.float64).reshape(and_span, or_span, num_terms), and_gates, or_gates, inputs_np_unshifted)
    filtered_coeffs = filter_coeffs(np.asarray(coeffs, dtype=np.float64).reshape(and_span, or_span, num_terms), and_gates, or_gates, inputs_np_unshifted)
